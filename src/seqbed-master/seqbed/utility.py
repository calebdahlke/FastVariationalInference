...
class MutualInformation(Utility):
    ...
    log_r_obs: np.ndarray
        Array of LFIRE log-ratios obtained for each prior sample through the
    ...
    def compute(self, d, numsamp=10000, evaltype="robust", verbose=True):
        ...
        elif self.evalmethod == "mm":
            infobj = inference.MOMENT_MATCH(
                d, self.prior_samples, self.weights, self.simobj
            )
            self.utils = infobj.eval(numsamp=numsamp)
    ...
    def compute_final(self, d_opt, y_obs, num_cores=1):
        ...
        if (self.evalmethod == "lfire") or (self.evalmethod == "mm"):
            ...
            tmp_bl = Parallel(n_jobs=int(num_cores))(
                delayed(infobj._logistic_regression)(p, K=3) for p in self.prior_samples
            )
            self.b_obs = np.array(tmp_bl)

            # Compute ratios for each coefficient
            self.log_r_obs = np.array(
                [
                    (psi_obs.reshape(1, -1) @ b[1:] + b[0])[0][0]
                    for b in self.b_obs
                ]
            )        

            return self.log_r_obs, self.b_obs
...
if __name__ == '__main__':
    print('Main called in utility.py!')