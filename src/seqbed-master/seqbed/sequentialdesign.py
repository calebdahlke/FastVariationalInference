...
class SequentialBED:
    ...
    def _objective(self, d):
        ...
        """
        if (self.utiltype == "MI") or (self.utiltype == "MI_MM"):
    ...
    def optimisation(
        ...
        for n in range(self.iter_start + 1, self.iter_start + n_iter + 1):
            ...
            # DEBUG: show objective 
            # myBopt.plot_acquisition(label_x="(d) Time", label_y="(negative) Mutual Information")
            ...
            if (self.utiltype == "MI") or (self.utiltype == "MI_MM"):
                ...
                log_r_obs, b_obs = self.utilobj.compute_final(
                    d_opt, y_obs, num_cores=self.num_cores
                )

                # Compute final utility value
                u_final = robust.MEstimator(log_r_obs)

                self.savedata = {
                    "d_opt": d_opt,
                    "y_obs": y_obs,
                    "r_obs": np.exp(log_r_obs),
                    "b_obs": b_obs,
                    "u_final": u_final,
                    "neff": self._ESS(),
                ...
                log_weights = np.log( self.weights ) + log_r_obs
...
class SequentialBED_checkpoint(SequentialBED):
    ...
    def __init__(
        ...
        if (self.utiltype == "MI") or (self.utiltype == "MI_MM"):
            ...
            self.weights = self.data["weights"] * self.data["r_obs"]  ## (!!) Do in log-domain and make numerically stable
            self.smc = smc
        elif self.utiltype == "Precision":
            # Assign weights from data
...
if __name__ == '__main__':
    print('Main called in sequentialdesign.py!')