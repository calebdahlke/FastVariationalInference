...
class SIRModel(Simulator):
    ...
    def generate_data(self, d, p):
        ...
        if platform.system() == "Linux":
            libname = "./seqbed/libsimulate_sir.so"
        else:   
            libname = "./seqbed/libsimulate_sir.dll"
        c_lib = ctypes.CDLL(libname)

        # call C++ code
        S, I, R = ctypes.c_int(self.S0), ctypes.c_int(self.I0), ctypes.c_int(self.R0)
        c_lib.simulate_sir(
            ctypes.byref(S), ctypes.byref(I), ctypes.byref(R),
        ...
        return np.array([S.value, I.value, R.value])
...
if __name__ == '__main__':
    print('Main called in simulator.py!')